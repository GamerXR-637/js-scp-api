    ...
    try {
        ...
        paragraphs.each((_, paragraph) => {
            if (!stop) {
                ...
                if (strongTag.length) {
                    ...
                    currentValue = $(paragraph).text().substring(currentKey.length).trim();
        ...
        return {
            ...
            more_info: Object.fromEntries(
                Object.entries(resultDict).filter(([key]) =>
                    !['object_class', 'special_containment_procedures', 'description', 'name'].includes(key)
                )
            )
        };
    } catch (error) {
